    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-green-100">
        ...
        <p className="mt-4 text-center text-sm text-gray-600">
          ...
          <button onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')} className="text-green-600 font-semibold hover:text-green-700">{authMode === 'login' ? 'Sign Up' : 'Sign In'}</button>
        </p>
      </div>
    </div>
  );
